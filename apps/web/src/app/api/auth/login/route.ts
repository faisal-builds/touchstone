...
export async function POST(req: NextRequest) {
  const { email, password, org_slug } = await req.json();
  const res = await fetch(`${CONTROL_PLANE_URL}/v1/auth/login`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ email, password, org_slug: org_slug || null }),
    cache: "no-store",
  });
  if (!res.ok) {
