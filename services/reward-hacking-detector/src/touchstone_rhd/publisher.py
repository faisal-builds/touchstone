"""Publisher for reward-hacking events (robustness.evaluated).

A Protocol allows a no-broker collecting fake in tests; the Kafka implementation
is used in production. The RHD consumes the control-plane topic (for auto-eval on
verifier registration) and publishes to the reward-hacking topic, so there is no
feedback loop.
"""
